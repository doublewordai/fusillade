...
mod tests {
    ...
    #[sqlx::test]
    async fn test_claim_requests_fair_across_models_over_time(pool: sqlx::PgPool) {
        let http_client = Arc::new(MockHttpClient::new());
        let manager = PostgresRequestManager::with_client(pool.clone(), http_client);

        // Create file with 30 requests (10 per model)
        ...
        let file_id = manager
            .create_file("fairness-test".to_string(), None, templates)
            .await
            .unwrap();
        ...
        // Track how many requests we get from each model over multiple claim iterations
        let mut model_counts = std::collections::HashMap::new();
        model_counts.insert("model-a".to_string(), 0);
        ...
        for iteration in 0..20 {
            ...
            for request in claimed {
                *model_counts.get_mut(&request.data.model).unwrap() += 1;
            }
        }
        ...
        for (model, count) in &model_counts {
            ...
            assert!(
                *count <= 10,
                "Model {} should not exceed its 10 available requests, got {}",
                model,
                count
            );
        }
